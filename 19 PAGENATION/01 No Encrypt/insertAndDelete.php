<?php  
	...
	// var_dump($totalData);
	$totalData = count(query("SELECT * FROM fruits"));

	
	// $totalPage = $totalData / $totalDataOnPage;
	// $totalPage = round($totalData / $totalDataOnPage);
	// $totalPage = floor($totalData / $totalDataOnPage);
	$totalPage = ceil($totalData / $totalDataOnPage);
	// var_dump($totalPage);
	...
	$firstData = ( $totalDataOnPage * $activePage ) - $totalDataOnPage;


	// $fruits = query("SELECT * FROM fruits LIMIT 1, 2");
	// $fruits = query("SELECT * FROM fruits LIMIT 5, 3");
	$fruits = query("SELECT * FROM fruits LIMIT $firstData, $totalDataOnPage");
...
<body>
	...
	<!-- To The Left -->
	<?php if( $activePage > 1) : ?>
		<a href="?page=<?php echo $activePage - 1; ?>">&lt;</a>
	<?php endif; ?>
